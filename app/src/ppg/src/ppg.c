...
#define RMSSD_MOV_AVG_SIZE 30

LOG_MODULE_REGISTER(ppg, LOG_LEVEL_DBG);

static void sampling_tmr_cb(struct k_timer *p_tmr);
static inline float ms_to_bpm_mov_avg(int64_t ms);
static inline float ms_to_rmssd_mov_avg(int64_t ms);
...
static float rmssd_mov_avg_buf[RMSSD_MOV_AVG_SIZE];
static ring_buffer_t rmssd_mov_avg_ring_buf;

static atomic_t current_hr_bpm;
static atomic_t current_rmssd;
...
void ppg_start_sampling(void)
{
    ...
    ring_buffer_init(&rmssd_mov_avg_ring_buf, rmssd_mov_avg_buf, RMSSD_MOV_AVG_SIZE);
...
{
    return (uint32_t) atomic_get(&current_rmssd);
}
...
static void sampling_tmr_cb(struct k_timer *p_tmr)
{
    ...
    if (checkForBeat(new_sample))
    {
        ...
        if (last_beat_ms != 0)
        {
            ...
            bpm = ms_to_bpm_mov_avg(diff_ms);
            rmssd = ms_to_rmssd_mov_avg(diff_ms);

            atomic_set(&current_hr_bpm, (atomic_val_t) bpm);
            atomic_set(&current_rmssd, (atomic_val_t) rmssd);
        }
...
static inline float ms_to_rmssd_mov_avg(int64_t ms)
{
    float ibi_buf[RMSSD_MOV_AVG_SIZE];
    int num_ibi;
    float ibi_diff;
    float ibi_diff_sq_sum = 0.0f;
    float rmssd;

    ring_buffer_put(&rmssd_mov_avg_ring_buf, (float) ms);
    LOG_DBG("Inserted %f into rmssd buf", (float) ms);
    if (rmssd_mov_avg_ring_buf.num_items > 1)
    {
        num_ibi = ring_buffer_copy_inorder(&rmssd_mov_avg_ring_buf, ibi_buf);
